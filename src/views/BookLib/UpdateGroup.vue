        ...
        <el-form label-width="80px;" label-position="left">
            ...
            <el-form-item label="选择轮播图">
                <el-select v-model="carousels" filterable multiple clearable placeholder="请选择">
                    <el-option v-for="item in carousList" :key="item.pic_url" :label="item.description == ''? item.id : item.description" :value="item.pic_url" :disabled="item.status == 1? false: true">
                    </el-option>
                </el-select>
            </el-form-item>
        </el-form>
        <div class="operation">
            <el-button type="primary" @click="onSubmint">更新</el-button>
...
export default {
    ...
    methods: {
        getsingleinfo: function () {
            var self = this;
            var params_id = Number.parseInt(this.$route.params.id);
            this.axios.post(host.data,{
                action_name: "get_article_params",
                data: {
                    id: params_id
                }
            })
            .then(function (res){
                ...
                var data = res.data;
                if(data.code == 0){
                    self.QR_Url = data.data.qrcode_url;
                    self.groupName = data.data.title;
                    for(var i = 0; i < data.data.carousel.length; i++){
                        if( data.data.carousel[i] != null){
                            self.carousels.push(data.data.carousel[i]);
                        }
                    }
                }
            })
        ...
        onSubmint: function () {
            ...
            for(var i = 0; i < self.carousels.length; i++){
                for(var j=0; j < self.carousList.length; j++){
                    if(self.carousels[i] == self.carousList[j].pic_url){
                        carr.push(self.carousList[j].id)
                    }
                }
            }
            var id = Number.parseInt(this.$route.params.id);
            this.axios.post(host.data,{
                action_name: "update_article_params",
                data: {
                    id: id,
