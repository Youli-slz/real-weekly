    ...
    methods: {
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            self.axios.post('http://wxmpx.gatao.cn/realtech/login',{RealtechLoginSearch: self.RealtechLoginSearch, type: 4})   // ,{withCredentials: true} 用于跨域请求  RealtechLoginSearch: {username:self.ruleForm.user, password: self.ruleForm.pass}}
              .then((response) => {
                var data = response.data;
                if(data.code == 0){ 
                  self.setCookie("token",data.msg.token,60*60*30);
                   self.$router.push('/carousels?user=' + self.ruleForm.user);                 
                }
                else {
                  alert(data.msg);
                }
