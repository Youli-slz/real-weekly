    ...
    methods: {
      submitForm(formName) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            self.axios.post('http://wxmp.gatao.cn/realtech/login',{RealtechLoginSearch: self.RealtechLoginSearch, type: 4})   // ,{withCredentials: true} 用于跨域请求  RealtechLoginSearch: {username:self.ruleForm.user, password: self.ruleForm.pass}}
              .then((response) => {
